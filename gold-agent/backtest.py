...
def run_backtest(config: dict | None = None, use_cache: bool = True) -> dict:
    ...
    try:
        with _quiet():
            raw = yf.download("GC=F", period="60d", interval="1h",
    ...
    if df_full is None or len(df_full) < MIN_ROWS + 5:
        try:
            with _quiet():
                raw = yf.download(
                    "GC=F", period="6mo", interval="1d", progress=False, auto_adjust=True)
            if isinstance(raw.columns, pd.MultiIndex):
                raw.columns = raw.columns.get_level_values(0)
            raw.index.name = "Date"
            df_full = raw[["Open", "High", "Low", "Close", "Volume"]].dropna()
    ...
    print("=" * 60)
    print(
        f"  Period   : {df_full.index[MIN_ROWS-1].date()} -> {df_full.index[-1].date()}")
    ...
    print(
        f"  Cache    : {'YES (same price data)' if use_cache else 'NO (fresh fetch)'}")
    ...
    for i in range(MIN_ROWS - 1, len(df_full)):
        ...
        for pos in open_positions:
            ...
            highest = max(
                pos.get("highest_price", pos["entry_price"]), price_thb)
        ...
        if (decision == "HOLD" and
                ...
                _window_used < 2):
            with _quiet(), patch.object(fetch_module, "get_gold_price", return_value=window):
                agent = run_agent(quota_pressure=True,
                                  failsafe_pressure=True, config=config)
            decision = agent["decision"]
            confidence = agent["confidence"]
            reasoning = agent["reasoning"]
            if decision == "HOLD":
        ...
        # ── BUY ───────────────────────────────────────────────────────────────
            ...
            if balance_thb >= MIN_BALANCE_THB:
                ...
                _window_trades[date_str_today][candle_window] = _window_trades[date_str_today].get(
                    candle_window, 0) + 1
            else:
                action = "SKIP"
        ...
        unrealized = sum((p["size_bw"] * price_thb) - p["cost_thb"]
                         for p in open_positions)
        equity = round(
            balance_thb + sum(p["size_bw"] * price_thb for p in open_positions), 2)
    ...
    avg_loss = abs(sum(t["pnl_thb"] for t in losses)) / \
    ...
    _session_usd = round(
        _cost_after["total_cost_usd"] - _cost_before["total_cost_usd"], 6)
    _session_thb = round(
        _cost_after["total_cost_thb"] - _cost_before["total_cost_thb"], 4)
    ...
    print("=" * 60)
    print(
        f"  Period     : {daily_log[0]['date'] if daily_log else '?'}  ->  {daily_log[-1]['date'] if daily_log else '?'}  ({calendar_days}d)")
    print(
        f"  Candles    : {candles_run} x {interval_used}  |  Trades : {len(closed_trades)}")
    print("")
    print("  PERFORMANCE")
    print(f"  {'Return':<14}: {ret_sign}{ret_pct:.2f}%   (B{INITIAL_BALANCE_THB:,.0f} -> B{final_equity:,.2f})")
    print(
        f"  {'Total P&L':<14}: {pnl_sign}B{total_pnl:,.2f}   Fees: B{total_fees:,.2f}")
    print("")
    print("  TRADE STATS")
    print(
        f"  {'Win Rate':<14}: {win_rate:.1f}%   ({len(wins)} wins / {len(losses)} losses)")
    ...
    print(
        f"  Trade log   : data/backtest_trades.csv ({len(closed_trades)} trades)")
    print("=" * 60)
    print("")
